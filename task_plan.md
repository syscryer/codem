...
## 当前规划：Agent 安装更新与版本感知

### 目标

- 将设置导航和页标题中的“Agent 与模型”统一改为“Agent 设置”。
- 移除实验性 Agent 开关，Claude Code、Codex、Grok Build、OpenCode 均按正式能力展示。
- 为每个 Agent 展示安装状态、当前版本、最新版本和可更新状态，并在安装或更新后自动重新探测。
- npm 类 Agent 官方源下载发生网络失败时，自动使用可信国内镜像重试；非网络错误不得被镜像重试掩盖。

### 阶段

| 阶段 | 状态 | 说明 |
| --- | --- | --- |
| 1. 现状与 CC Switch 调研 | completed | 已核对现有诊断/安装接口、版本类型、实验开关引用和 CC Switch 版本/镜像策略 |
| 2. 跨层契约与后端实现 | completed | 已增加最新版本与更新状态探测，镜像只处理严格网络错误，安装命令保持白名单 |
| 3. Agent 设置 UI | completed | 已改名、移除实验开关，并完成安装/更新版本状态和反馈 |
| 4. 测试与真实验收 | completed | Rust/TypeScript/前端全量测试通过，宽屏和 900px 真实浏览器验收通过 |
| 5. Trellis 收口 | completed | 验证结果与 Grok 官方更新器限制已记录；未提交、未推送 |

### 边界

- 不修改普通聊天供应商和模型机制。
- 不执行前端传入的任意安装命令，继续使用后端 Provider 白名单。
- 不把包名、权限、安装脚本等确定性错误误判为网络问题并镜像重试。
- 不保存或展示任何 Agent 凭据。
...
| 错误 | 尝试次数 | 解决方案 |
| --- | --- | --- |
| 首次规划补丁依赖已漂移的 `findings.md` 上下文 | 1 | 补丁整体拒绝且无文件写入；读取真实尾部后拆分为精确补丁 |
| 初次 `cargo fmt --check` 报告 Rust 排版差异 | 1 | 测试补齐后统一执行 `cargo fmt`，最终格式门禁通过 |
| Tauri watcher 重启后端期间 Playwright 记录短暂 500 | 1 | 等待 3001 新进程稳定后刷新页面，控制台恢复 0 error/0 warning |
