  ...
  return (
    <section className="settings-page-section">
      ...
      <div className="settings-actions">
        <button type="button" className="ghost-button" onClick={() => setDraft(settings)}>清除未保存修改</button>
        <button type="button" className="primary-button" onClick={() => void save()}><Save size={14} />保存设置</button>
        <button type="button" className="ghost-button" disabled={testing} onClick={() => void testConnection()}><RefreshCw size={14} className={testing ? 'spin' : ''} />测试连接</button>
      </div>
    </section>
  );
}
