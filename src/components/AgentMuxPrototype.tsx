...
function buildSkillText(agentItems: AgentRecord[], cliPath: string, appDataDir: string) {
  const profiles = agentItems.flatMap((agent) => agent.profiles.filter((profile) => profile.status === 'available').map((profile) => `- profileId: ${profile.id}; agentId: ${agent.id}; providerId: ${agentProviderId(agent.id) ?? 'unsupported'}; model: ${profile.model}; reasoningEffort: ${profile.reasoningEffort ?? 'model-default'}; channelId: ${profile.channelId ?? 'system'}; level: ${profile.level}; role: ${profile.role}; tags: ${profile.tags.join('、') || '无'}`));
...
function MonitorView({ agents, runs: items, projects, selected, events, liveTurn, onSelect, onCancel }: { agents: AgentRecord[]; runs: RunRecord[]; projects: ProjectSummary[]; selected?: RunRecord; events: AgentMuxRunEvent[]; liveTurn?: AgentMuxConversationTurn; onSelect: (id: string) => void; onCancel: (run: RunRecord) => Promise<void> }) {
  ...
  return <div className="agent-mux-page agent-mux-split-page agent-mux-monitor-page"><div className="agent-mux-list-panel"><div className="agent-mux-list-header"><div><h2>运行监控</h2><p>按工作区查看所有 Agent Mux 调用。</p></div><button type="button" className="agent-mux-icon-button" title="刷新" onClick={() => selected && onSelect(selected.id)}><RefreshCw size={15} /></button></div><div className="agent-mux-filter-bar"><span className="active">全部 {items.length}</span><span>运行中 {runningCount}</span><span>失败 {failedCount}</span></div><div className="agent-mux-run-list">{workspaceGroups.length > 0 ? workspaceGroups.map((group) => { const expanded = !collapsedWorkspaceKeys.has(group.key); return <section className="agent-mux-workspace-group" key={group.key}><button type="button" className="agent-mux-workspace-group-head" aria-expanded={expanded} onClick={() => toggleWorkspaceGroup(group.key)}><ChevronRight className={`agent-mux-workspace-chevron${expanded ? ' is-expanded' : ''}`} size={13} /><FolderOpen size={14} /><span><strong>{group.name}</strong><small title={group.path}>{group.path}</small></span><b>{group.runs.length}</b></button>{expanded ? group.runs.map((run) => { const agentId = agentIdForRun(run, agents); return <button type="button" key={run.id} className={`agent-mux-run-item${selected?.id === run.id ? ' selected' : ''}`} onClick={() => onSelect(run.id)}><RunIcon status={run.status} /><span className="agent-mux-agent-mark small" data-provider={agentId}><AgentProviderIcon providerId={agentProviderId(agentId) ?? agentId} size={15} /></span><span><strong>{run.target}</strong><small>{run.caller} · {run.profile}</small></span><time>{run.started}</time></button>; }) : null}</section>; }) : <EmptyState title="暂无运行记录" detail="真实 Agent 调用产生记录后，会在这里显示状态和输出。" />}</div></div><div className="agent-mux-detail-panel">{selected ? <><div className="agent-mux-detail-heading"><div><span className="agent-mux-detail-kicker">{selected.id} · {selected.skill}</span><div className="agent-mux-detail-title"><span className="agent-mux-agent-mark" data-provider={selectedAgentId}><AgentProviderIcon providerId={agentProviderId(selectedAgentId) ?? selectedAgentId} size={17} /></span><h2>{selected.target}</h2></div><div className="agent-mux-run-prompt"><span>调用提示词</span><p>{selected.prompt || '旧记录未保存原始提示词'}</p></div></div>{selected.status === 'running' && selected.providerRunId ? <button type="button" className="agent-mux-stop-button" title="取消运行" aria-label="取消运行" onClick={() => void onCancel(selected)}><Square size={13} fill="currentColor" /></button> : null}</div><div className="agent-mux-run-summary"><Metric label="状态" value={runLabel(selected.status)} accent={selected.status === 'running'} success={selected.status === 'completed'} /><Metric label="调用方" value={selected.caller} /><Metric label="运行配置" value={selected.profile} /><Metric label="耗时" value={selected.duration} /></div><section className="agent-mux-detail-section"><div className="agent-mux-section-title"><div><h3>输出</h3><p>复用聊天的统一 Agent 事件解析与展示。</p></div><span className="agent-mux-live"><span />{selected.status === 'running' ? '实时' : '已记录'}</span></div><AgentMuxRunLog run={selected} events={events} liveTurn={liveTurn} /></section></> : <EmptyState title="请选择运行记录" detail="暂无可查看的 Agent Mux 运行。" />}</div></div>;
}

function AgentMuxRunLog({ run, events, liveTurn }: { run: RunRecord; events: AgentMuxRunEvent[]; liveTurn?: AgentMuxConversationTurn }) {
...
function EmptyState({ title, detail }: { title: string; detail: string }) { return <div className="agent-mux-empty-state"><strong>{title}</strong><span>{detail}</span></div>; }
...
function agentProviderId(agentId: string): AgentProviderId | null { return agentId === 'codex' ? OPENAI_CODEX_PROVIDER_ID : agentId === 'claude' ? CLAUDE_CODE_PROVIDER_ID : agentId === 'grok' ? GROK_BUILD_PROVIDER_ID : agentId === 'pi' ? PI_AGENT_PROVIDER_ID : agentId === 'opencode' ? OPENCODE_PROVIDER_ID : null; }
