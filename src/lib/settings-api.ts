...
export function normalizeAgentRuntimeSettings(value: unknown): AgentRuntimeSettings {
  const record = isRecord(value) ? value : {};
  const dshToolsMode = record.dshToolsMode === 'code' || record.dshToolsMode === 'both'
    ? record.dshToolsMode
    : 'native';
  return {
    defaultProviderId: normalizeAgentProviderId(record.defaultProviderId),
    dshProfile: normalizeLimitedString(record.dshProfile, 64) || 'headless',
    dshAgentPreset: normalizeLimitedString(record.dshAgentPreset, 64) || 'standard',
    dshToolsMode,
  };
}
