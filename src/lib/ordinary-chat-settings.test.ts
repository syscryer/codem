...
const agentChannelSettingsSource = readFileSync(new URL('../components/settings/AgentChannelSettings.tsx', import.meta.url), 'utf8');
...
test('普通聊天思考开关与等级选择职责分离并使用中性主题状态', () => {
  assert.match(composerSource, /className="permission-trigger ordinary-context-trigger ordinary-thinking-trigger"/);
  assert.match(composerSource, /aria-pressed=\{ordinaryThinkingEnabled\}/);
  assert.match(composerSource, /setReasoningMenuOpen\(false\);\s*void onToggleOrdinaryThinking\?\.\(\);/);
  assert.match(composerSource, /className="ordinary-thinking-level-trigger"/);
  assert.match(composerSource, /aria-label="选择思考等级"/);
  assert.doesNotMatch(composerSource, /if \(ordinaryThinkingEnabled\) setReasoningMenuOpen/);
  assert.match(stylesSource, /\.ordinary-thinking-control\.active\s*\{[\s\S]*?var\(--app-text-strong[\s\S]*?var\(--app-hover/);
  assert.match(stylesSource, /\.model-menu\.ordinary-reasoning-menu\s*\{[\s\S]*?width:\s*min\(180px[\s\S]*?max-height:\s*none/);
  assert.doesNotMatch(stylesSource, /\.ordinary-thinking-trigger\.active\s*\{[\s\S]*?--app-accent/);
});
...
test('普通聊天供应商入口跳转到 Agent 渠道管理中的普通聊天', () => {
  assert.match(appSource, /onOpenAiSettings=\{\(\) => openAgentChannelSettings\('ordinary-chat'\)\}/);
  assert.match(agentChannelSettingsSource, /if \(nextProviderId === 'ordinary-chat'\) \{[\s\S]*?setOrdinaryChatActive\(true\)/);
  assert.match(agentChannelSettingsSource, /focusRequest\?\.requestId/);
  assert.doesNotMatch(appSource, /onOpenAiSettings=\{\(\) => openSettings\('aiProviders'\)\}/);
});
