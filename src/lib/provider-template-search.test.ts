...
test('Agent 渠道设置统一展示系统渠道并移除模型列表地址输入', () => {
  assert.match(agentChannelSettingsSource, /<strong>系统渠道<\/strong>/);
  assert.match(agentChannelSettingsSource, /agent-system-channel-readonly-grid/);
  assert.doesNotMatch(agentChannelSettingsSource, /模型列表地址（可选）/);
});
